...
FileStore.prototype._sendRequest = function (oRequest, fnRequestCallback) {
    ...
    console.log("option=" + me._oOptions.conn.useStrictSSL);
    oRequest.strictSSL(me._oOptions.conn.useStrictSSL);
    // oRequest.strictSSL(false);
    ...
    async.series([
        ...
        function (fnCallbackAsyncL1) {
            ...
            async.series([
			    ...
			    console.log("debug-L3-1");
                            var sFolder = aFolders.shift();

			    console.log("URL=" + me._constructBaseUrl() + '/' + encodeURIComponent(sFolder) + '/content');
				...
				console.log("debug3 STATUS_CODE=" + oResponse.statusCode);
				if (oResponse.statusCode !== util.HTTPSTAT.ok) {
				    console.log("HTTPSTATUS is ok");
				}
				console.log("debug4");
...
FileStore.prototype.syncFolder = function (sFolder, sModif, fnCallback) {
    ...
    me._sendRequest(oRequest, function (oResponse) {
	console.log("enter me._sendRequest");
        if (oResponse.error) {
